                        ...
                        <PopoverContent className='w-80 h-80 mt-4 z-[200]'>
                            {
                                <Command className='rounded-lg'>
                                        ...
                                        {(user?.role === "AGENCY_OWNER" ||
                                            ...
                                            && user?.Aggency && (
                                                <CommandGroup heading="Agency">
                                                    <CommandItem className='!bg-tranparent my-2 text-primary boerder-[1px] boerder-border p-2 rounded-md hover:!bg-muted cursor-pointer transition-all'>
                                                        {defaultOpen ? (
                                            ...
                                            {!!subAccounts ?
                                                subAccounts.map((subaccount) => (
                                                    <CommandItem key={subaccount.id}>
                                                        {defaultOpen ? (
                                                            <Link href={`/subaccount/${user?.subaccount?.id}`} className='flex gap-4 w-full h-full '>
                                                                <div className="relative w-16">
                                                                    <Image src={user?.subaccount?.subaccountLogo} alt='Agency Logo' fill className='rounded-md object-contain' />
                                                                </div>
                                                                <div className="flex flex-col flex-1">
                                                                    {user?.subaccount?.name}
                                                                    <span className='text-muted-foreground'>{user?.subaccount?.address}</span>
                                                                </div>
                                                            </Link>
                                                        ) : (
                                                            <SheetClose asChild>
                                    ...
                                    {(user?.role === "AGENCY_OWNER" ||
                                        user?.role === "AGENCY_ADMIN") && (
                                            <SheetClose>
                                                <Button className='w-full felx gap-2'
                                                    onClick={() => {
